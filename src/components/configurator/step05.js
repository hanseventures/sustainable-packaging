...
const Step05 = (props) => {
  ...
  const {configuratorJson} = useStaticQuery(graphql`
    query Step05Query {
      configuratorJson(progressbar: {eq: "5"}) {
        id
        title
        progressbar
        btnPrevious
    ...
    <>
      ...
      <ul className='list-none d-grid box trimmed-4 text-xs-center my-5'>
        {configuratorJson.nodes.map((node) => (
          <li
            key={node.id}
            className={props.form.step05ID === node.id ? 'p-3 active' : 'p-3'}
            onClick={() =>
              props.dispatchForm({
                type: "UPDATE_KEY_VALUES",
                value: { step05ID: node.id }
              })
            }>
            <img src="svg/streamline-icon-shipment-in-transit@46x46.svg" className="mb-2" />
            <span>{node.text}</span>
          </li>
        ))}
      </ul>
