...
const Step06 = (props) => {
  ...
  const {configuratorJson} = useStaticQuery(graphql`
    query Step06Query {
      configuratorJson(progressbar: {eq: "6"}) {
        id
        title
        progressbar
        btnPrevious
    ...
    <>
      ...
      <ul className='list-none d-grid box trimmed-4 text-xs-center my-5'>
        {configuratorJson.nodes.map((node) => (
          <li
            key={node.id}
            className={props.form.step06ID === node.id ? 'p-3 active' : 'p-3'}
            onClick={() =>
              props.dispatchForm({
                type: "UPDATE_KEY_VALUES",
                value: { step06ID: node.id }
              })
            }>
            <img src="svg/streamline-icon-accounting-coins-stack-1@46x46.svg" className="mb-2" />
            <span>{node.text}</span>
          </li>
        ))}
      </ul>
